...
sub setup {
  ...
  $self->{_ike_group} = $config->returnValue('ike-settings ike-group');
  $self->{_esp_group} = $config->returnValue('ike-settings esp-group');
...
sub setupOrig {
  ...
  $self->{_ike_group} = $config->returnOrigValue('ike-settings ike-group');
  $self->{_esp_group} = $config->returnOrigValue('ike-settings esp-group');
  my $pfx = 'ike-settings authentication x509';
...
sub get_ra_conn {
  ...
  return (undef, "IPsec authentication mode not defined")
    if (!defined($self->{_mode}));
  return (undef, "IPSec IKE group not defined")
    if (!defined($self->{_ike_group}));
  return (undef, "IPSec ESP group not defined")
    if (!defined($self->{_esp_group}));
  $ike_str = get_ike_proposals($self->{_ike_group});
  $esp_str = get_esp_proposals($self->{_esp_group});
